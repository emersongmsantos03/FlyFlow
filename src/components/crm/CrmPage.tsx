...
function ContactDrawer({ lead, state, onClose, onEdit, onDelete, onLose, onAttachReceipt, onGenerateProposal, onRegisterInteraction, onSendWhatsAppApproach, onSendEmail, onEmailQuote, onScheduleReturn, onRegisterDeposit, onDownloadQuote, onApproveQuote, onMarkPaymentPaid, onCreateProject, onCreateTask, onEditTask, onCompleteTask, onReopenTask, onDeleteTask }: {
  ...
  return createPortal(<>
    ...
    <aside className="crm-contact-drawer">
      <header className="border-b border-gray-200 bg-white p-4">
        <div className="flex items-start justify-between gap-3">
          <div className="flex min-w-0 items-start gap-3">
            <span className="avatar-chip drawer-avatar" data-tint={avatarTint(lead.id)}>{initials(displayName(lead))}</span>
            <div className="min-w-0"><p className="text-xs font-bold uppercase text-gray-500">Cliente 360° · oportunidade</p><h2 className="mt-1 truncate text-xl font-black text-gray-950">{displayName(lead)}</h2><p className="text-sm text-gray-500">{lead.fullName || lead.city}</p></div>
          </div>
          <button className="focus-ring rounded-lg p-2 hover:bg-gray-100" aria-label="Fechar" type="button" onClick={onClose}><X size={20} /></button>
        </div>
      ...
      ] as Array<[DrawerTab, string]>).map(([value, label]) => <button key={value} className={`min-h-11 whitespace-nowrap border-b-2 px-3 text-xs font-bold ${tab === value ? 'is-active' : 'border-transparent text-gray-500 hover:text-gray-800'}`} type="button" onClick={() => setTab(value)}>{label}</button>)}</nav>
      <div className="space-y-4 p-4">
        {tab === 'overview' ? <>
          <div className="grid grid-cols-2 gap-2">{[
            ['Valor potencial', formatCurrency(lead.estimatedValue), <CircleDollarSign key="icon" size={16} />, 'green'],
            ['Próxima ação', lead.nextContactAt ? formatDateTime(lead.nextContactAt) : 'Não definida', <CalendarDays key="icon" size={16} />, 'blue'],
            ['Serviço', lead.serviceInterest, <Briefcase key="icon" size={16} />, 'peach'],
            ['Origem', lead.source, <Globe2 key="icon" size={16} />, 'lavender'],
          ].map(([label, value, icon, tint]) => <div key={label as string} className="drawer-info-tile rounded-lg border border-gray-200 bg-gray-50 p-3"><span className="drawer-info-icon" data-tint={tint}>{icon}</span><span className="min-w-0"><p className="text-[0.68rem] font-bold uppercase text-gray-500">{label}</p><p className="mt-1 truncate text-sm font-black text-gray-950">{value}</p></span></div>)}</div>
